...
import { LogOut, Settings, Search, Filter } from "lucide-react"
...
export default function GymManagement() {
    ...
    const fetchClients = async () => {
      try {
        ...
        const clientList: Client[] = response.documents.map((doc: any) => ({
          ...
          isActive: new Date() <= new Date(doc.subscriptionEnd), // Update status based on current date
        }))
  ...
  const addClient = async () => {
    ...
    try {
      ...
      // Update local state with the actual Appwrite document ID
      const clientWithAppwriteId = {
        ...newClient,
        id: response.$id,
      }

      setClients([...clients, clientWithAppwriteId])
              ...
              <div className="flex items-center gap-4">
                ...
                {currentUser?.role === "admin" && (
                  ...
                  >
                    ...
                    {showUserManagement ? "Clients" : "Utilisateurs"}
                  </Button>
                )}
                ...
                <Button
                  onClick={logout }
                  variant="outline"
                  className="border-red-600 text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-all duration-300 bg-transparent"
                >
                ...
                <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        placeholder="Rechercher par nom, téléphone ou âge..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 focus:border-orange-500 focus:ring-orange-500/20 pl-10 w-full sm:w-80"
                      />
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Select value={filterStatus} onValueChange={setFilterStatus}>
                        <SelectTrigger className="bg-slate-700 border-slate-600 text-white focus:border-orange-500 focus:ring-orange-500/20 w-48">
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-400">
                    ...
                    {filterStatus !== "all" && (
                      <Badge variant="outline" className="border-slate-600 text-slate-300">
                        {filterStatus === "active" ? "Actifs" : "Expirés"}
                      </Badge>
                    )}
                    <Button
                      variant="ghost"
                      ...
                      className="text-slate-400 hover:text-white hover:bg-slate-700 transition-all duration-300"
                    >
                      Effacer les filtres
                    </Button>
                  </div>
              ...
              {filteredClients.length === 0 && clients.length > 0 && (
                <Card className="text-center py-16 bg-gradient-to-br from-slate-800 to-slate-700 border-slate-600 shadow-2xl">
                  <CardContent>
                    ...
                    <h3 className="text-2xl font-bold text-white mb-3">Aucun résultat trouvé</h3>
                    <p className="text-slate-400 mb-6 max-w-md mx-auto">
                      Aucun client ne correspond à vos critères de recherche ou de filtrage
                    </p>
                    <Button
                      onClick={() => {
                        setSearchTerm("")
